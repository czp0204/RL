...
option(Striker)
{
  ...
  state(turnToBall)
  {
    ...
    {// 如果看不到球的话
      ...
      if(theBallModel.estimate.position.angle() < 5_deg)//球相对于机器人的位置.角度
        goto walkToBall;
    ...
    action
    {
      ...
      WalkToTarget(Pose2f(150.f, 150.f, 150.f), Pose2f(theBallModel.estimate.position.angle(), 0.f, 0.f));
  ...
  state(walkToBall) // 每次踢完球 球不一定正对着机器人 自己调整位置
  {
    transition
    {
      if(theLibCodeRelease.timeSinceBallWasSeen > theBehaviorParameters.ballNotSeenTimeOut)
        goto searchForBall;
         OUTPUT_TEXT("找球");
      if(theBallModel.estimate.position.norm() < 500.f)//500.f  theBallModel.estimate.position.norm()
       OUTPUT_TEXT("对齐球");
        goto alignToGoal;
    }
    action
    {
      theHeadControlMode = HeadControl::lookForward;//7.8.18:26
      WalkToTarget(Pose2f(150.f, 150.f, 150.f), Pose2f(theBallModel.estimate.position.angle(), 0.f, 0.f));
  ...
  state(alignToGoal)// 将自己朝向对方的球门
  {
    transition
    {
      ...
      if(std::abs(theLibCodeRelease.angleToGoal) < 10_deg && std::abs(theBallModel.estimate.position.y()) < 100.f)
        goto alignBehindBall;//  theBallModel.estimate.position 本来就是相对于机器人的坐标
            }
    action
    {
       theHeadControlMode = HeadControl::lookForward;
      WalkToTarget(Pose2f(150.f, 150.f, 150.f), Pose2f(theLibCodeRelease.angleToGoal, theBallModel.estimate.position.x()-300.f, theBallModel.estimate.position.y()));//x-300.f

    }//Thegoal is球门 walk原来后边跟的都是绝对坐标
  }
  ...
  state(alignBehindBall)// 走到球后边 不好的地方是 每次丢掉球 都要重新找球 不如ready的写法
  {
    transition
    {
      if(theLibCodeRelease.timeSinceBallWasSeen > theBehaviorParameters.ballNotSeenTimeOut)
        goto searchForBall;
      if(theLibCodeRelease.between(theBallModel.estimate.position.y(), 20.f, 50.f)//在这两个值之间
    ...
    {
      theHeadControlMode = HeadControl::lookForward;
      WalkToTarget(Pose2f(150.f, 150.f, 150.f), Pose2f(theLibCodeRelease.angleToGoal, theBallModel.estimate.position.x() - 150.f, theBallModel.estimate.position.y() - 30.f));
    }
  }
